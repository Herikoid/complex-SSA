...
\vspace{1em}
\alert{Задача:} Извлечение сигнала $\tilde{\tX{S}} = F(\tX{X})$, $F$ используемый метод.\\
\vspace{1em}
\alert{Метод:} SSA (Singular Spectrum Analysis) [Golyandina et al., 2001] для вещественных рядов, CSSA (Complex Singular Spectrum Analysis) для комплексных.\\
\vspace{1em}
\alert{Проблема:} Что лучше, с точки зрения $\tilde{\tX{S}} - \tX{S}$,

SSA$(\tX{X}_{\Re}) + \iu \text{SSA}(\tX{X}_{\Im})$ или CSSA$(\tX{X}_{\Re} + \iu \tX{X}_{\Im})$?

\end{frame}

\begin{frame}{ведение: Обозначения}
Рассмотрим временной ряд $\tX{X}=(x_1, \ldots, x_{N})$, $L$ длина окна.\\
\vspace{1em}
...
Временной ряд $\tX{X}=(x_1, \ldots, x_{N})$, $L$ длина окна.
...
\alert{Модель:} $\tX{S}(\delta) = \tX{S} + \delta \tX{R}$ длины $N$ или $\mathbf{H}(\delta) = \mathbf{H} + \delta \mathbf{E}$, 
...
$\tX{F}^{(1)} = \mathcal{T}^{-1}_{L}(\mathbf{H}^{(1)})$ первый порядок ошибки восстановления.\\
...
Что является причиной, по которой можно не требовать независимость шума.
\end{frame}
...
\begin{block}{Замечание}
    Для сигнала $s_l = A\cos(2 \pi\omega l + \phi_1) + \iu B\cos(2 \pi\omega l + \phi_2)$, при сдвиге не $\pi / 2$, с возмущением $\tX{R}$ выполняется
    $$\mathbb{D}(\text{CSSA-}f^{(1)}_l) = \mathbb{D}(\text{SSA-}f^{(1)}_{\Re, l}) + \mathbb{D}(\text{SSA-}f^{(1)}_{\Im, l}).$$
\end{block}
...
$\text{SSA-}f^{(1)}_{\Re, l} = \mathcal{T}^{-1}_{L}(\mathbf{H}^{(1)})_{l}.$\\
\vspace{1em}
...
\begin{frame}{Заключение}
    \begin{itemize}
        \item При совпадении траекторных пространств комплексного ряда для первого порядка было показано, что результаты CSSA и SSA совпадают.
        \item Из численных экспериментов, в случае случайного возмущения, первый порядок адекватно описывает полную ошибку.
        \item $s_l = A\cos(2 \pi\omega l + \phi_1) + \iu B\cos(2 \pi\omega l + \phi_2)$, сдвиг не $\pi / 2$
        $$\mathbb{D}(\text{CSSA-}f^{(1)}_l) = \mathbb{D}(\text{SSA-}f^{(1)}_{\Re, l}) + \mathbb{D}(\text{SSA-}f^{(1)}_{\Im, l}).$$
        $s_l = Ae^{\pm \iu(2 \pi\omega l + \phi_1)}$
        $$\mathbb{D}(\text{CSSA-}f^{(1)}_l) \stackrel{?}{=} \frac{1}{2}[\mathbb{D}(\text{SSA-}f^{(1)}_{\Re, l}) + \mathbb{D}(\text{SSA-}f^{(1)}_{\Im, l})].$$
        \item Из численных экспериментов, в случае выброса первый порядок адекватно оценивает полную ошибку при $L = \alpha N$.
        \item Для $s_l = c_1 + \iu c_2$ с выбросом, была получена аналитическая формула первого порядка. При $L = \alpha N$ ошибка стремится к $0$.
    \end{itemize}
\end{frame}
