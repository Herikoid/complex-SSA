...
В процессе реализации комплексной версии алгоритма, была найдена и исправлена опечатка, допущенная в \cite{Chen}, а именно, в формуле \eqref{taskB} был пропущен знак эрмитового сопряжения для $\mathbf{v}_j$, что приводило к тому, что при решении задачи ошибочной искалось $\mathbf{V}^\mathrm{H}$, вместо $\mathbf{V}$ 
...
\begin{remark} \label{rm:probl_rand}
	В силу своей случайности, такое возмущение не всегда будет удовлетворять \eqref{eq:main_cond}. А значит и теорема \ref{th:sum} не всегда будет применима, даже для требуемого сигнала. Однако мы будем считать, что возмущение $\tR$ имеет достаточно малую дисперсию, при которой вероятность подобного исхода пренебрежимо мала.
\end{remark}
...
Что означает выполнение \eqref{eq:main_cond}. Продемонстрируем выполнение условия на конкретных примерах.
