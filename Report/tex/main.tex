...
\begin{statement}
	Для сигнала случая комплексной экспоненты, с возмущением $\tR$ выполняется \footnote{утверждение ли?}
	\begin{equation} \label{eq:expdisp}
		\mathbb{D}(f^{(1)}_l) \stackrel{?}{=} \frac{1}{2}[\mathbb{D}(f^{(1)}_{\Re, l}) + \mathbb{D}f^{(1)}_{\Im, l})].
	\end{equation}
\end{statement}

Формула \eqref{eq:expdisp} была проверена числено.\footnote{надо ли приводить численный эксперимент? если надо, то как оформлять, непонятно.}
...
\subsubsection{Сохранение RMSE} \label{ss:RMSEinv}

Обозначим $RMSE = \sqrt{\frac{1}{N}\sum_{l = 1}^{N} (f^{(1)}_l)^2}$ для ряда $\tX = \tS + \tR$.\footnote{не уверен, что этот раздел вообще нужен}

Возмущение для выброса $a + \iu b$ можно записать как  $\tR = a\tG + \iu b\tG$, где $\frac{1}{a}\Re(\tR) = \frac{1}{b}\Im(\tR)  = \tG$.
...
\begin{proof}
	...
	По теореме \ref{th:sum} и формуле \eqref{eq:main}
	$$f^{(1)}_l = a\mathcal{H}(\tG, \Re(\tS)) + ib\mathcal{H}(\tG, \Im(S)) = c_l(a + \iu b).$$
...
Результат для одной из реализаций шума представлен на рис.~\ref{fig:harm_noise}.
...
Из графика видно, что ошибки практически совпадают даже при маленьких $L$ и $N$. Аналогичные численные эксперименты подтверждают, что для комплексной экспоненты также есть такое совпадение\footnote{надо ли приводить?}.

\subsection{Случай константных сигналов с выбросом}

Был рассмотрен пример с сигналом $s_l = 1 + \iu 1$, с возмущением в виде выброса $a_1 + \iu a_2 = 10 + \iu 10$ на позиции $k = L - 1$. 

Результаты представлены в таблице~\ref{tab:const_outl}.
...
Аналогичные численные эксперименты показывают, что при расположении выброса в середине ряда результаты качественно совпадают, при $L = N/2$ различие стремится к $0$, при $L = 20$ не стремится к $0$\footnote{надо ли приводить?}.
