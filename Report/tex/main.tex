...
\footnote{В алгоритмах странно выглядят конкретные числа $\varepsilon = 10^{-4}$, максимальное число итераций $N_{iter} = 10$. Если они нужны, укажите это там, где нужны при применении алгоритма.}

В приведённой реализации $\mathbf{V}(0)$ инициализируется при помощи сингулярного разложения, но, согласно~\cite{KeKanade}, инициализация может быть произведена при помощи любой матрицы требуемого размера с сохранением сходимости. \footnote{А что там конкретно говорится про сходимость, есть какой-то теоретический результат?}
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{$\mathbf{Y} \in \mathbb{C}^{L\times K}$~--- траекторная матрица ряда, $r$~--- ранг сигнала; параметр весовой функции $\alpha$; параметры критерия остановки: $\varepsilon$, максимальное число итераций $N_{iter}$}
\KwOut{$\mathbf{\hat{Y}} = \mathbf{U}\mathbf{V}^\mathrm{H}$~--- проекция траекторной матрицы на множество матриц ранга, не превосходящего $r$}
