...
\KwOut{$\mathbf{\hat{Y}} = \mathbf{U}\mathbf{V}^\mathrm{H}$~--- проекция траекторной матрицы на множество матриц ранга, не превосходящего $r$}
 ...
 \caption{Последовательный метод построения $\mathbb{L}_1$-проектора на множество матриц ранга, не превосходящего $r$ (L1)}
\end{algorithm}
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{$\mathbf{Y} \in \mathbb{C}^{L\times K}$~--- траекторная матрица ряда, $r$~--- ранг сигнала; параметр весовой функции $\alpha$; алгоритм обновления матрицы $\Sigma$; параметры критерия остановки: $\varepsilon$, максимальное число итераций $N_{iter}$}
\KwOut{$\mathbf{\hat{Y}} = \mathbf{U}\mathbf{V}^\mathrm{H}$~--- проекция траекторной матрицы на множество матриц ранга, не превосходящего $r$}
...
$\Sigma$ предлагается взять, состоящей из одинаковых элементов $\sigma_{ij} = \sigma = 1.4826 \med {|\tR-\med {|\tR|}|}$, где $\tR$~--- это вектор, составленный из всех элементов матрицы остатков $\mathbf{R} = \{r_{ij}\}_{i,j=1}^{L,K}$, то есть
\begin{equation*}
...
Обозначим метод CSSA(SSA) за Basic, как базовый метод для робастных модификаций.
...
Рассмотрим те же ряды, но с вещественными выбросом величины $5|x_i|$ на позиции $N/2$.
...
\begin{statement} \label{st:main}
	...
	\end{equation}
	$\forall \delta \in (-\delta_0, \delta_0)$, где $\mu_{\min}$~--- минимальное сингулярное число $\mathbf{H}$, то  
...
Возмущение для выброса $a_1 + \iu a_2$ можно записать как  $\tR = (a_1 + \iu a_2)\tG$, где $\tG$~--- возмущение $1$ на $k$-м месте.

\begin{statement}\label{st:RMSEinv}
	Пусть возмущение для выброса $a_1 + \iu a_2$ на позиции $k$ достаточно мало.
	Тогда $|f_l^{(1)}|$ для ряда с сигналом $\tS$ и выбросом $a_1 + \iu a_2$ на позиции $k$, и  ряда с сигналом $\tS$ и выбросом $b_1 + \iu b_2$ на позиции $k$, таким что $|b_1 + \iu b_2| = |a_1 + \iu a_2|$, совпадают $\forall l$.
\end{statement}
\begin{proof}
	...
	$$|f^{(1)}_l| = |(a_1 + \iu a_2)c_l| = |a_1 + \iu a_2||c_l|. $$
	...
	$$|f^{(1)}_l| = |(b_1 + \iu b_2)c_l| = |b_1 + \iu b_2||c_l| = |a_1 + \iu a_2||c_l|. $$
\end{proof}
...
И получаем формулы для поэлементного выражения первого порядка ошибки восстановления
...
\begin{table}[H]
	\begin{center}
		\caption{Соотношение ошибок в точке максимального различия, $L = N/2$.}
