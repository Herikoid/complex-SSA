...
\footnote{А что такое n и p внутри алгоритмов?}
...
\footnote{У меня здесь был комментарий, какой конкретно метод что нужно в заголовке таблицы написать, какой конкретно метод (или я просто вписала, но вы должны были увидеть по diff). Это относится и к всем след. таблицам.}
\footnote{Сразу еще общий комментарий - не должно быть одинаковых заголовков. Там, где приводите вещ.и мнимую часть, например, пишите сначала название примера. Например, `Аддитивный ряд. Мнимая часть.' А значение M, если оно везде одинаковое, не надо писать - это технический параметр, его просто укажите в комментарии к примеру.}
...
\footnote{Слово классический лучше не использовать в таком контексте. Можно говорить о методе из \cite{Trickett.etal2012}}
...
\footnote{К этому разделу нужно написать короткое резюме по примерам выше. Что они подтвердили, что требуется еще исследовать.}
...
\footnote{У вас же это написано в первой главе, так что сошлитесь и туда: ..., см. утверждение ...}
...
\end{remark}
\footnote{Что-то я перестала понимать предыдущий вариант этого замечания. Следствие не про комплексную экспоненты, а тут про комплексную экспоненту.}
...
Показанное совпадение и теорема \ref{th:sum} объясняют эффект,
\footnote{надо парой слов обозначить, что за эффект}
 наблюдаемый нами в разделе \ref{sec:comp}.
...
Численные результаты показывают, что для случая зашумленных гармоник первый порядок адекватно оценивает полную ошибку восстановления сигнала в каждой точке при любых рассматриваемых параметрах сигналов, хотя на краях разность ошибок существенно медленнее стремится к нулю.
\footnote{а разве для зашумленного ряда это показывали?}
