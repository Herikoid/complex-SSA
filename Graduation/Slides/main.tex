...
\begin{frame}{Часть \RomanNumeralCaps{2}: Структура}
	Структура доклада:
	\begin{enumerate}
		\item Обозначения
		\item Первый порядок ошибки для общего случая
		\item Пример: зашумлённые гармоники
		\item Пример: константный сигнал с выбросом
		\item Сравнение первого порядка и полной ошибок
	\end{enumerate}
\end{frame}
